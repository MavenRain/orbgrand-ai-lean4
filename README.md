...
[`doc-gen4`](https://github.com/leanprover/doc-gen4).  The
`doc-gen4` package is loaded only under the `dev` configuration so
that the default `lake build` does not pull in the documentation
toolchain.  To build the documentation locally:

```sh
lake -Kenv=dev update
lake -Kenv=dev build OrbgrandAi:docs
```
