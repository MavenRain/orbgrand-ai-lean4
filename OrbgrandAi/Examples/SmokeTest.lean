...
/-- `delayTapMatrix` thirty-first sub-diagonal. -/
example {n_s : Nat} {p : Nat} (paths : Fin p -> DelayTapPath)
    (f_s : SamplingFreq) (i j : Fin n_s) (h : i.val = j.val + 31) :
    delayTapMatrix n_s paths f_s i j
      = delayTapImpulseResponse paths f_s { toNat := 31 } :=
  delayTapMatrix_thirty_first_subdiag paths f_s i j h

/-- `delayTapMatrix` thirty-second sub-diagonal. -/
example {n_s : Nat} {p : Nat} (paths : Fin p -> DelayTapPath)
    (f_s : SamplingFreq) (i j : Fin n_s) (h : i.val = j.val + 32) :
    delayTapMatrix n_s paths f_s i j
      = delayTapImpulseResponse paths f_s { toNat := 32 } :=
  delayTapMatrix_thirty_second_subdiag paths f_s i j h

/-- `delayTapMatrix` thirty-third sub-diagonal. -/
example {n_s : Nat} {p : Nat} (paths : Fin p -> DelayTapPath)
    (f_s : SamplingFreq) (i j : Fin n_s) (h : i.val = j.val + 33) :
    delayTapMatrix n_s paths f_s i j
      = delayTapImpulseResponse paths f_s { toNat := 33 } :=
  delayTapMatrix_thirty_third_subdiag paths f_s i j h
...
/-- `dicode` bandwidth widens to 28. -/
example {n_s : Nat} (sigma : NoisePower) (rho : CorrelationCoefficient) :
    (dicode n_s sigma rho).bandwidth 28 :=
  dicode_bandwidth_twenty_eight sigma rho

/-- `dicode` bandwidth widens to 29. -/
example {n_s : Nat} (sigma : NoisePower) (rho : CorrelationCoefficient) :
    (dicode n_s sigma rho).bandwidth 29 :=
  dicode_bandwidth_twenty_nine sigma rho

/-- `dicode` bandwidth widens to 30. -/
example {n_s : Nat} (sigma : NoisePower) (rho : CorrelationCoefficient) :
    (dicode n_s sigma rho).bandwidth 30 :=
  dicode_bandwidth_thirty sigma rho
...
/-- `rfView` bandwidth widens to 36. -/
example {n_s : Nat} (rowTaps : Fin n_s -> RFViewTaps) (sigma : NoisePower) :
    (rfView n_s rowTaps sigma).bandwidth 36 :=
  rfView_bandwidth_thirty_six rowTaps sigma

/-- `rfView` bandwidth widens to 37. -/
example {n_s : Nat} (rowTaps : Fin n_s -> RFViewTaps) (sigma : NoisePower) :
    (rfView n_s rowTaps sigma).bandwidth 37 :=
  rfView_bandwidth_thirty_seven rowTaps sigma

/-- `rfView` bandwidth widens to 38. -/
example {n_s : Nat} (rowTaps : Fin n_s -> RFViewTaps) (sigma : NoisePower) :
    (rfView n_s rowTaps sigma).bandwidth 38 :=
  rfView_bandwidth_thirty_eight rowTaps sigma
...
/-- `LinearIsi.bandwidth` widens by twenty-three. -/
example {n_s : Nat} {ch : LinearIsi n_s} {b : Nat} (h : ch.bandwidth b) :
    ch.bandwidth (b + 23) :=
...
/-- `LinearIsi.bandwidth` widens by twenty-four. -/
example {n_s : Nat} {ch : LinearIsi n_s} {b : Nat} (h : ch.bandwidth b) :
    ch.bandwidth (b + 24) :=
...
/-- `LinearIsi.bandwidth` widens by twenty-five. -/
example {n_s : Nat} {ch : LinearIsi n_s} {b : Nat} (h : ch.bandwidth b) :
    ch.bandwidth (b + 25) :=
  LinearIsi.bandwidth_succ_succ_succ_succ_succ_succ_succ_succ_succ_succ_succ_succ_succ_succ_succ_succ_succ_succ_succ_succ_succ_succ_succ_succ_succ h
...
/-- `SamplingFreq.mk?` round-trips through `7`. -/
example : SamplingFreq.mk? 7 = Except.ok ⟨7, Nat.ofNat_pos⟩ :=
  SamplingFreq.mk?_seven

/-- `BlockSize.mk?` round-trips through `8`. -/
example : BlockSize.mk? 8 = Except.ok ⟨8, Nat.succ_pos 7⟩ :=
  BlockSize.mk?_eight

/-- `CodewordLength.mk?` round-trips through `8`. -/
example : CodewordLength.mk? 8 = Except.ok ⟨8, Nat.succ_pos 7⟩ :=
  CodewordLength.mk?_eight
...
/-- `cov1_lag` at lag `23`. -/
example (sigma : NoisePower) (rho : CorrelationCoefficient) :
    cov1_lag sigma rho 23 = sigma.val * rho.val ^ 23 :=
  cov1_lag_twenty_three sigma rho

/-- `cov1_lag` at lag `-23`. -/
example (sigma : NoisePower) (rho : CorrelationCoefficient) :
    cov1_lag sigma rho (-23) = sigma.val * rho.val ^ 23 :=
  cov1_lag_neg_twenty_three sigma rho

/-- `cov1_lag` at lag `24`. -/
example (sigma : NoisePower) (rho : CorrelationCoefficient) :
    cov1_lag sigma rho 24 = sigma.val * rho.val ^ 24 :=
  cov1_lag_twenty_four sigma rho
...
/-- `entropyRate2` at the block size `n_s = 21`. -/
example (sigma : NoisePower) (rho1 rho2 : CorrelationCoefficient) :
    entropyRate2 sigma rho1 rho2 21
      = (1 / 2 : Real)
          * Real.log (2 * Real.pi * Real.exp 1 * sigma.val)
        + (1 / (2 * ((21 : Nat) : Real)))
            * Real.log
                (- (rho2.val - 1) ^ 19
                    * (1 - 2 * rho1.val ^ 2 + rho2.val) ^ 19
                  / (rho1.val ^ 2 - 1) ^ 18) :=
  entropyRate2_at_twenty_one_eq_log_form sigma rho1 rho2

/-- `entropyRate2` at the block size `n_s = 22`. -/
example (sigma : NoisePower) (rho1 rho2 : CorrelationCoefficient) :
    entropyRate2 sigma rho1 rho2 22
      = (1 / 2 : Real)
          * Real.log (2 * Real.pi * Real.exp 1 * sigma.val)
        + (1 / (2 * ((22 : Nat) : Real)))
            * Real.log
                (- (rho2.val - 1) ^ 20
                    * (1 - 2 * rho1.val ^ 2 + rho2.val) ^ 20
                  / (rho1.val ^ 2 - 1) ^ 19) :=
  entropyRate2_at_twenty_two_eq_log_form sigma rho1 rho2

/-- `entropyRate2` at the block size `n_s = 23`. -/
example (sigma : NoisePower) (rho1 rho2 : CorrelationCoefficient) :
    entropyRate2 sigma rho1 rho2 23
      = (1 / 2 : Real)
          * Real.log (2 * Real.pi * Real.exp 1 * sigma.val)
        + (1 / (2 * ((23 : Nat) : Real)))
            * Real.log
                (- (rho2.val - 1) ^ 21
                    * (1 - 2 * rho1.val ^ 2 + rho2.val) ^ 21
                  / (rho1.val ^ 2 - 1) ^ 20) :=
  entropyRate2_at_twenty_three_eq_log_form sigma rho1 rho2
...
example {u v : Nat -> Real} {c1 c2 : Real}
    ...
    (h : c2 < c1) :
    limsupEntropyRate v < liminfInformationRate u :=
  limsupEntropyRate_lt_liminfInformationRate_of_eventually_const_mono hu hv h
...
example {u v : Nat -> Real} {c1 c2 : Real}
    ...
    (h : c1 < c2) :
    liminfInformationRate u < limsupEntropyRate v :=
  liminfInformationRate_lt_limsupEntropyRate_of_eventually_const_mono hu hv h
...
example {u v : Nat -> Real} {c : Real}
    ...
    (hv : ∀ᶠ n in Filter.atTop, v n = c) :
    liminfInformationRate u = liminfInformationRate v :=
  liminfInformationRate_eq_liminfInformationRate_of_eventually_const_pair hu hv
...
/-- QPSK exceedance at `(2, 3)` is strictly positive. -/
example : 0 < qpsk.exceed 2 3 := qpsk_exceed_two_three_pos

/-- QPSK exceedance at `(3, 0)` is strictly positive. -/
example : 0 < qpsk.exceed 3 0 := qpsk_exceed_three_zero_pos

/-- QPSK exceedance at `(3, 1)` is non-zero. -/
example : qpsk.exceed 3 1 ≠ 0 := qpsk_exceed_three_one_ne_zero
...
/-- Pointwise double-negation self-XOR vanishes. -/
example {n : Nat} (a : Codeword n) (i : Fin n) :
    Codeword.xor (-a) (-a) i = 0 :=
  Codeword.neg_xor_neg_self_apply a i

/-- Negation of zero codeword is zero. -/
example {n : Nat} :
    -(0 : Codeword n) = 0 :=
  Codeword.neg_zero
...
/-- `landslideBucket` non-membership three indices before the witness. -/
example {n : Nat} (pi : ReliabilityRank n) {w : Nat}
    {e : Fin n -> Bool} (h1 : landslideBucket pi (w + 3) e) :
    ¬ landslideBucket pi w e :=
  landslideBucket_not_of_mem_pred_pred_pred_self pi h1

/-- `landslideBucket` three-step conjunction-form disjointness. -/
example {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool} :
    ¬ (landslideBucket pi w e /\ landslideBucket pi (w + 3) e) :=
  landslideBucket_disjoint_succ_succ_succ_self pi

/-- `landslideBucket` three-step conjunction-form disjointness, swapped order. -/
example {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool} :
    ¬ (landslideBucket pi (w + 3) e /\ landslideBucket pi w e) :=
  landslideBucket_disjoint_pred_pred_pred_self pi
...
/-- Vacuous codebook with empty pattern list at budget 10. -/
example {n_s b numCandidates : Nat} (Y : Codeword n_s) :
    orbgrandAi (b := b) (numCandidates := numCandidates)
      Y (fun _ => false) (AbandonmentBudget.mk 10)
      ([] : List (Fin (n_s / b) -> Fin numCandidates)) = none :=
  orbgrandAi_empty_codebook_nil_mk_ten Y

/-- Vacuous codebook returns `none` at budget `11`. -/
example {n_s b numCandidates : Nat}
    (Y : Codeword n_s)
    (patterns : List (Fin (n_s / b) -> Fin numCandidates)) :
    orbgrandAi (b := b) (numCandidates := numCandidates)
      Y (fun _ => false) (AbandonmentBudget.mk 11) patterns = none :=
  orbgrandAi_empty_codebook_mk_eleven Y patterns

/-- Vacuous codebook with empty pattern list at budget 11. -/
example {n_s b numCandidates : Nat} (Y : Codeword n_s) :
    orbgrandAi (b := b) (numCandidates := numCandidates)
      Y (fun _ => false) (AbandonmentBudget.mk 11)
      ([] : List (Fin (n_s / b) -> Fin numCandidates)) = none :=
  orbgrandAi_empty_codebook_nil_mk_eleven Y
...
/-- AR(2) recurrence step at index 66. -/
example (phi1 phi2 z1 z2 : Complex) :
    ar2 phi1 phi2 z1 z2 66
      = phi1 * ar2 phi1 phi2 z1 z2 65
        + phi2 * ar2 phi1 phi2 z1 z2 64 :=
  ar2_sixty_six phi1 phi2 z1 z2

/-- AR(2) recurrence step at index 67. -/
example (phi1 phi2 z1 z2 : Complex) :
    ar2 phi1 phi2 z1 z2 67
      = phi1 * ar2 phi1 phi2 z1 z2 66
        + phi2 * ar2 phi1 phi2 z1 z2 65 :=
  ar2_sixty_seven phi1 phi2 z1 z2

/-- AR(2) recurrence step at index 68. -/
example (phi1 phi2 z1 z2 : Complex) :
    ar2 phi1 phi2 z1 z2 68
      = phi1 * ar2 phi1 phi2 z1 z2 67
        + phi2 * ar2 phi1 phi2 z1 z2 66 :=
  ar2_sixty_eight phi1 phi2 z1 z2
...
/-- `kendallTau` iff form of weak-inequality propagation via zero right-distance. -/
example {numPatterns : Nat} (a b c : QueryOrder numPatterns) {n : Nat}
    (hbc : kendallTau b c = 0) :
    kendallTau a b <= n ↔ kendallTau a c <= n :=
  kendallTau_le_iff_le_of_eq_zero_right a b c hbc

/-- `kendallTau` symm-anchored iff form of strict-inequality propagation. -/
example {numPatterns : Nat} (a b c : QueryOrder numPatterns) {n : Nat}
    (hab : kendallTau a b = 0) :
    kendallTau c a < n ↔ kendallTau c b < n :=
  kendallTau_lt_iff_lt_of_eq_zero_symm a b c hab

/-- `kendallTau` symm-anchored iff form of weak-inequality propagation. -/
example {numPatterns : Nat} (a b c : QueryOrder numPatterns) {n : Nat}
    (hab : kendallTau a b = 0) :
    kendallTau c a <= n ↔ kendallTau c b <= n :=
  kendallTau_le_iff_le_of_eq_zero_symm a b c hab
...
/-- Nine is a valid sampling frequency. -/
example : SamplingFreq.mk? 9 = Except.ok ⟨9, Nat.ofNat_pos⟩ :=
  SamplingFreq.mk?_nine

/-- Thirty-seventh sub-diagonal of `delayTapMatrix`. -/
example {n_s p : Nat} (paths : Fin p -> DelayTapPath)
    (f_s : SamplingFreq) (i j : Fin n_s) (h : i.val = j.val + 37) :
    delayTapMatrix n_s paths f_s i j
      = delayTapImpulseResponse paths f_s { toNat := 37 } :=
  delayTapMatrix_thirty_seventh_subdiag paths f_s i j h

/-- Dicode bandwidth widens to 34. -/
example {n_s : Nat} (sigma : NoisePower) (rho : CorrelationCoefficient) :
    (dicode n_s sigma rho).bandwidth 34 :=
  dicode_bandwidth_thirty_four sigma rho

/-- Doubling of `receive` in the signal with a split noise. -/
example {n_s : Nat} (ch : LinearIsi n_s) (X N1 N2 : SymbolVector n_s) :
    ch.receive (X + X) (N1 + N2) = ch.receive X N1 + ch.receive X N2 :=
  LinearIsi.receive_signal_self_noise_add ch X N1 N2

/-- RFView bandwidth widens to 42. -/
example {n_s : Nat} (rowTaps : Fin n_s -> RFViewTaps) (sigma : NoisePower) :
    (rfView n_s rowTaps sigma).bandwidth 42 :=
  rfView_bandwidth_forty_two rowTaps sigma
...
example {u v : Nat -> Real} {c1 c2 : Real}
    ...
    (h : c1 <= c2) :
    limsupEntropyRate u <= limsupEntropyRate v :=
  limsupEntropyRate_le_limsupEntropyRate_of_eventually_const_mono hu hv h

/-- Zero-sigma determinant vanishes at `n_s = 8`. -/
example (rho1 rho2 : CorrelationCoefficient) :
    cov2DetFormula ⟨0, le_refl 0⟩ rho1 rho2 8 = 0 :=
  cov2DetFormula_zero_sigma_eight rho1 rho2

/-- Boundary case `n_s = 27` of `entropyRate2`. -/
example (sigma : NoisePower) (rho1 rho2 : CorrelationCoefficient) :
    entropyRate2 sigma rho1 rho2 27
      = (1 / 2 : Real)
          * Real.log (2 * Real.pi * Real.exp 1 * sigma.val)
        + (1 / (2 * ((27 : Nat) : Real)))
            * Real.log
                (- (rho2.val - 1) ^ 25
                    * (1 - 2 * rho1.val ^ 2 + rho2.val) ^ 25
                  / (rho1.val ^ 2 - 1) ^ 24) :=
  entropyRate2_at_twenty_seven_eq_log_form sigma rho1 rho2

/-- `cov1_lag` at lag 26. -/
example (sigma : NoisePower) (rho : CorrelationCoefficient) :
    cov1_lag sigma rho 26 = sigma.val * rho.val ^ 26 :=
  cov1_lag_twenty_six sigma rho

/-- QPSK exceedance at `(0, 2)` is non-zero. -/
example : qpsk.exceed 0 2 ≠ 0 :=
  qpsk_exceed_zero_two_ne_zero

/-- Pointwise double negation. -/
example {n : Nat} (a : Codeword n) (i : Fin n) :
    (-(-a)) i = a i :=
  Codeword.neg_neg_apply a i

/-- Four-step conjunction-form disjointness, swapped order. -/
example {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool} :
    ¬ (landslideBucket pi (w + 4) e /\ landslideBucket pi w e) :=
  landslideBucket_disjoint_pred_pred_pred_pred_self pi

/-- Vacuous codebook on the empty pattern list at budget 13. -/
example {n_s b numCandidates : Nat}
    (Y : Codeword n_s) :
    orbgrandAi (b := b) (numCandidates := numCandidates)
      Y (fun _ => false) (AbandonmentBudget.mk 13)
      ([] : List (Fin (n_s / b) -> Fin numCandidates)) = none :=
  orbgrandAi_empty_codebook_nil_mk_thirteen Y

/-- Recurrence step at index 72. -/
example (phi1 phi2 z1 z2 : Complex) :
    ar2 phi1 phi2 z1 z2 72
      = phi1 * ar2 phi1 phi2 z1 z2 71
        + phi2 * ar2 phi1 phi2 z1 z2 70 :=
  ar2_seventy_two phi1 phi2 z1 z2

/-- Zero left-distance gives an iff for exact values. -/
example {numPatterns : Nat}
    (a b c : QueryOrder numPatterns) {n : Nat}
    (hab : kendallTau a b = 0) :
    kendallTau c a = n ↔ kendallTau c b = n :=
  kendallTau_eq_iff_eq_of_eq_zero_symm a b c hab

/-- Septuple zero-perturbation collapses to identity. -/
example {n_s : Nat} (h : ChannelMatrix n_s) :
...
/-- Ten is a valid block size. -/
example : BlockSize.mk? 10 = Except.ok ⟨10, Nat.succ_pos 9⟩ :=
  BlockSize.mk?_ten

/-- Thirty-eighth sub-diagonal of `delayTapMatrix`. -/
example {n_s p : Nat} (paths : Fin p -> DelayTapPath)
    (f_s : SamplingFreq) (i j : Fin n_s) (h : i.val = j.val + 38) :
    delayTapMatrix n_s paths f_s i j
      = delayTapImpulseResponse paths f_s { toNat := 38 } :=
  delayTapMatrix_thirty_eighth_subdiag paths f_s i j h

/-- Dicode bandwidth widens to 35. -/
example {n_s : Nat} (sigma : NoisePower) (rho : CorrelationCoefficient) :
    (dicode n_s sigma rho).bandwidth 35 :=
  dicode_bandwidth_thirty_five sigma rho

/-- Doubling of `receive` in the noise with a split signal. -/
example {n_s : Nat} (ch : LinearIsi n_s) (X1 X2 N : SymbolVector n_s) :
    ch.receive (X1 + X2) (N + N) = ch.receive X1 N + ch.receive X2 N :=
  LinearIsi.receive_signal_add_noise_self ch X1 X2 N

/-- RFView bandwidth widens to 43. -/
example {n_s : Nat} (rowTaps : Fin n_s -> RFViewTaps) (sigma : NoisePower) :
    (rfView n_s rowTaps sigma).bandwidth 43 :=
  rfView_bandwidth_forty_three rowTaps sigma

/-- Two-valued log-inverse-density. -/
example : limsupEntropyRate (fun _ : Nat => (2 : Real)) = 2 :=
  limsupEntropyRate_two

/-- Zero-sigma determinant vanishes at `n_s = 9`. -/
example (rho1 rho2 : CorrelationCoefficient) :
    cov2DetFormula ⟨0, le_refl 0⟩ rho1 rho2 9 = 0 :=
  cov2DetFormula_zero_sigma_nine rho1 rho2

/-- Boundary case `n_s = 28` of `entropyRate2`. -/
example (sigma : NoisePower) (rho1 rho2 : CorrelationCoefficient) :
        ...
        + (1 / (2 * ((28 : Nat) : Real)))
            * Real.log
                (- (rho2.val - 1) ^ 26
                  ...
                  / (rho1.val ^ 2 - 1) ^ 25) :=
  entropyRate2_at_twenty_eight_eq_log_form sigma rho1 rho2

/-- `cov1_lag` at lag -26 is `sigma * rho^26`. -/
example (sigma : NoisePower) (rho : CorrelationCoefficient) :
    cov1_lag sigma rho (-26) = sigma.val * rho.val ^ 26 :=
  cov1_lag_neg_twenty_six sigma rho

/-- QPSK exceedance at `(1, 2)` is non-zero. -/
example : qpsk.exceed 1 2 ≠ 0 :=
  qpsk_exceed_one_two_ne_zero

/-- Triple negation collapses to a single negation. -/
example {n : Nat} (a : Codeword n) : -(-(-a)) = -a :=
  Codeword.neg_neg_neg a

/-- Bucket non-membership five indices past the witness. -/
example {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool}
    (h1 : landslideBucket pi w e) :
    ¬ landslideBucket pi (w + 5) e :=
  landslideBucket_not_of_mem_succ_succ_succ_succ_succ_self pi h1

/-- Vacuous codebook at budget 14. -/
example {n_s b numCandidates : Nat}
    (Y : Codeword n_s)
    (patterns : List (Fin (n_s / b) -> Fin numCandidates)) :
    orbgrandAi (b := b) (numCandidates := numCandidates)
      Y (fun _ => false) (AbandonmentBudget.mk 14) patterns = none :=
  orbgrandAi_empty_codebook_mk_fourteen Y patterns

/-- Recurrence step at index 73. -/
example (phi1 phi2 z1 z2 : Complex) :
    ar2 phi1 phi2 z1 z2 73
      = phi1 * ar2 phi1 phi2 z1 z2 72
        + phi2 * ar2 phi1 phi2 z1 z2 71 :=
  ar2_seventy_three phi1 phi2 z1 z2
...
example {numPatterns : Nat}
    ...
    (hab : kendallTau a b = 0) :
    kendallTau a c = n ↔ kendallTau b c = n :=
  kendallTau_eq_iff_eq_of_eq_zero_left a b c hab

/-- Octuple zero-perturbation collapses to identity. -/
example {n_s : Nat} (h : ChannelMatrix n_s) :
  ...
  perturbChannel_perturbChannel_perturbChannel_perturbChannel_perturbChannel_perturbChannel_perturbChannel_perturbChannel_zero_zero_zero_zero_zero_zero_zero_zero h

/-! ## Round 51 additions -/

/-- Ten is a valid codeword length. -/
example : CodewordLength.mk? 10 = Except.ok ⟨10, Nat.succ_pos 9⟩ :=
  CodewordLength.mk?_ten

/-- Thirty-ninth sub-diagonal of `delayTapMatrix`. -/
example {n_s p : Nat} (paths : Fin p -> DelayTapPath)
    (f_s : SamplingFreq) (i j : Fin n_s) (h : i.val = j.val + 39) :
    delayTapMatrix n_s paths f_s i j
      = delayTapImpulseResponse paths f_s { toNat := 39 } :=
  delayTapMatrix_thirty_ninth_subdiag paths f_s i j h

/-- Dicode bandwidth widens to 36. -/
example {n_s : Nat} (sigma : NoisePower) (rho : CorrelationCoefficient) :
    (dicode n_s sigma rho).bandwidth 36 :=
  dicode_bandwidth_thirty_six sigma rho

/-- Swapped-signal doubling of `receive` in the noise. -/
example {n_s : Nat} (ch : LinearIsi n_s) (X1 X2 N : SymbolVector n_s) :
    ch.receive (X2 + X1) (N + N) = ch.receive X2 N + ch.receive X1 N :=
  LinearIsi.receive_signal_swap_noise_self ch X1 X2 N

/-- RFView bandwidth widens to 44. -/
example {n_s : Nat} (rowTaps : Fin n_s -> RFViewTaps) (sigma : NoisePower) :
    (rfView n_s rowTaps sigma).bandwidth 44 :=
  rfView_bandwidth_forty_four rowTaps sigma

/-- Three-valued log-inverse-density. -/
example : limsupEntropyRate (fun _ : Nat => (3 : Real)) = 3 :=
  limsupEntropyRate_three

/-- Zero-sigma determinant vanishes at `n_s = 10`. -/
example (rho1 rho2 : CorrelationCoefficient) :
    cov2DetFormula ⟨0, le_refl 0⟩ rho1 rho2 10 = 0 :=
  cov2DetFormula_zero_sigma_ten rho1 rho2

/-- Boundary case `n_s = 29` of `entropyRate2`. -/
example (sigma : NoisePower) (rho1 rho2 : CorrelationCoefficient) :
    entropyRate2 sigma rho1 rho2 29
      = (1 / 2 : Real)
          * Real.log (2 * Real.pi * Real.exp 1 * sigma.val)
        + (1 / (2 * ((29 : Nat) : Real)))
            * Real.log
                (- (rho2.val - 1) ^ 27
                    * (1 - 2 * rho1.val ^ 2 + rho2.val) ^ 27
                  / (rho1.val ^ 2 - 1) ^ 26) :=
  entropyRate2_at_twenty_nine_eq_log_form sigma rho1 rho2

/-- `cov1_lag` at lag 27 is `sigma * rho^27`. -/
example (sigma : NoisePower) (rho : CorrelationCoefficient) :
    cov1_lag sigma rho 27 = sigma.val * rho.val ^ 27 :=
  cov1_lag_twenty_seven sigma rho

/-- QPSK exceedance at `(1, 0)` is non-zero. -/
example : qpsk.exceed 1 0 ≠ 0 :=
  qpsk_exceed_one_zero_ne_zero

/-- Pointwise triple negation. -/
example {n : Nat} (a : Codeword n) (i : Fin n) : (-(-(-a))) i = (-a) i :=
  Codeword.neg_neg_neg_apply a i

/-- Bucket non-membership five indices before the witness. -/
example {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool}
    (h1 : landslideBucket pi (w + 5) e) :
    ¬ landslideBucket pi w e :=
  landslideBucket_not_of_mem_pred_pred_pred_pred_pred_self pi h1

/-- Vacuous codebook on the empty pattern list at budget 14. -/
example {n_s b numCandidates : Nat} (Y : Codeword n_s) :
    orbgrandAi (b := b) (numCandidates := numCandidates)
      Y (fun _ => false) (AbandonmentBudget.mk 14)
      ([] : List (Fin (n_s / b) -> Fin numCandidates)) = none :=
  orbgrandAi_empty_codebook_nil_mk_fourteen Y

/-- Recurrence step at index 74. -/
example (phi1 phi2 z1 z2 : Complex) :
    ar2 phi1 phi2 z1 z2 74
      = phi1 * ar2 phi1 phi2 z1 z2 73
        + phi2 * ar2 phi1 phi2 z1 z2 72 :=
  ar2_seventy_four phi1 phi2 z1 z2
...
example {numPatterns : Nat}
    ...
    (hbc : kendallTau b c = 0) :
    kendallTau a b = n ↔ kendallTau a c = n :=
  kendallTau_eq_iff_eq_of_eq_zero_right a b c hbc

/-- Nonuple zero-perturbation collapses to identity. -/
example {n_s : Nat} (h : ChannelMatrix n_s) :
    perturbChannel (perturbChannel (perturbChannel (perturbChannel (perturbChannel (perturbChannel (perturbChannel (perturbChannel (perturbChannel h 0) 0) 0) 0) 0) 0) 0) 0) 0 = h :=
...
/-- Ten hertz is a valid sampling frequency. -/
example : SamplingFreq.mk? 10 = Except.ok ⟨10, Nat.ofNat_pos⟩ :=
  SamplingFreq.mk?_ten

/-- Fortieth sub-diagonal of `delayTapMatrix`. -/
example {n_s p : Nat} (paths : Fin p -> DelayTapPath)
    (f_s : SamplingFreq) (i j : Fin n_s) (h : i.val = j.val + 40) :
    delayTapMatrix n_s paths f_s i j
      = delayTapImpulseResponse paths f_s { toNat := 40 } :=
  delayTapMatrix_fortieth_subdiag paths f_s i j h

/-- Dicode bandwidth widens to 37. -/
example {n_s : Nat} (sigma : NoisePower) (rho : CorrelationCoefficient) :
    (dicode n_s sigma rho).bandwidth 37 :=
  dicode_bandwidth_thirty_seven sigma rho

/-- Swapped-noise doubling of `receive` in the signal. -/
example {n_s : Nat} (ch : LinearIsi n_s) (X N1 N2 : SymbolVector n_s) :
    ch.receive (X + X) (N2 + N1) = ch.receive X N2 + ch.receive X N1 :=
  LinearIsi.receive_noise_swap_signal_self ch X N1 N2

/-- RFView bandwidth widens to 45. -/
example {n_s : Nat} (rowTaps : Fin n_s -> RFViewTaps) (sigma : NoisePower) :
    (rfView n_s rowTaps sigma).bandwidth 45 :=
  rfView_bandwidth_forty_five rowTaps sigma

/-- Four-valued log-inverse-density. -/
example : limsupEntropyRate (fun _ : Nat => (4 : Real)) = 4 :=
  limsupEntropyRate_four

/-- Zero-sigma determinant vanishes at `n_s = 11`. -/
example (rho1 rho2 : CorrelationCoefficient) :
    cov2DetFormula ⟨0, le_refl 0⟩ rho1 rho2 11 = 0 :=
  cov2DetFormula_zero_sigma_eleven rho1 rho2

/-- Boundary case `n_s = 30` of `entropyRate2`. -/
example (sigma : NoisePower) (rho1 rho2 : CorrelationCoefficient) :
    entropyRate2 sigma rho1 rho2 30
      = (1 / 2 : Real)
          * Real.log (2 * Real.pi * Real.exp 1 * sigma.val)
        + (1 / (2 * ((30 : Nat) : Real)))
            * Real.log
                (- (rho2.val - 1) ^ 28
                    * (1 - 2 * rho1.val ^ 2 + rho2.val) ^ 28
                  / (rho1.val ^ 2 - 1) ^ 27) :=
  entropyRate2_at_thirty_eq_log_form sigma rho1 rho2

/-- `cov1_lag` at lag -27 is `sigma * rho^27`. -/
example (sigma : NoisePower) (rho : CorrelationCoefficient) :
    cov1_lag sigma rho (-27) = sigma.val * rho.val ^ 27 :=
  cov1_lag_neg_twenty_seven sigma rho

/-- QPSK exceedance at `(2, 3)` is non-zero. -/
example : qpsk.exceed 2 3 ≠ 0 :=
  qpsk_exceed_two_three_ne_zero

/-- Quadruple negation is the identity. -/
example {n : Nat} (a : Codeword n) : -(-(-(-a))) = a :=
  Codeword.neg_neg_neg_neg a

/-- Five-step conjunction-form disjointness. -/
example {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool} :
    ¬ (landslideBucket pi w e /\ landslideBucket pi (w + 5) e) :=
  landslideBucket_disjoint_succ_succ_succ_succ_succ_self pi

/-- Vacuous codebook at budget 15. -/
example {n_s b numCandidates : Nat} (Y : Codeword n_s)
    (patterns : List (Fin (n_s / b) -> Fin numCandidates)) :
    orbgrandAi (b := b) (numCandidates := numCandidates)
      Y (fun _ => false) (AbandonmentBudget.mk 15) patterns = none :=
  orbgrandAi_empty_codebook_mk_fifteen Y patterns

/-- Recurrence step at index 75. -/
example (phi1 phi2 z1 z2 : Complex) :
    ar2 phi1 phi2 z1 z2 75
      = phi1 * ar2 phi1 phi2 z1 z2 74
        + phi2 * ar2 phi1 phi2 z1 z2 73 :=
  ar2_seventy_five phi1 phi2 z1 z2

/-- Zero right-distance gives an iff for exact values (symm-left-anchored). -/
example {numPatterns : Nat} (a b c : QueryOrder numPatterns) {n : Nat}
    (hbc : kendallTau b c = 0) :
    kendallTau b a = n ↔ kendallTau c a = n :=
  kendallTau_eq_iff_eq_of_eq_zero_symm_left a b c hbc

/-- Decuple zero-perturbation collapses to identity. -/
example {n_s : Nat} (h : ChannelMatrix n_s) :
