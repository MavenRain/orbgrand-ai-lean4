...
/-- *QPSK off-diagonal at `(3, 1)`.*  Witnessed by `(3 : Fin 4) ≠ 1`
    via `Fin.val`, one `Nat.succ.inj` peel, then `Nat.succ_ne_zero`. -/
theorem qpsk_exceed_three_one : qpsk.exceed 3 1 = 1 :=
  qpsk_exceed_diff
    (fun h : (3 : Fin 4) = 1 =>
      Nat.succ_ne_zero 1 (Nat.succ.inj (congrArg Fin.val h)))

/-- *QPSK off-diagonal at `(3, 2)`.*  Symmetric companion to
    `qpsk_exceed_two_three`; witnessed by `(3 : Fin 4) ≠ 2` via
    `Fin.val`, two `Nat.succ.inj` peels, then `Nat.succ_ne_zero`. -/
theorem qpsk_exceed_three_two : qpsk.exceed 3 2 = 1 :=
  qpsk_exceed_diff
    (fun h : (3 : Fin 4) = 2 =>
...
/-- *QPSK exceedance is strictly positive iff symbols differ.*
    QPSK-named specialisation of `Constellation.exceed_pos_iff_ne`
    at `cs = qpsk`. -/
theorem qpsk_exceed_pos_iff_ne (s s_hat : Fin 4) :
...
/-- *QPSK off-diagonal at `(2, 0)`.*  Symmetric companion to
    `qpsk_exceed_zero_two`, derived via `qpsk_exceed_symm`. -/
...
/-- *QPSK off-diagonal at `(3, 2)` is strictly positive.*  Concrete
    `pos`-corollary of `qpsk_exceed_three_two`. -/
theorem qpsk_exceed_three_two_pos : 0 < qpsk.exceed 3 2 :=
  lt_of_lt_of_eq zero_lt_one qpsk_exceed_three_two.symm

/-- *QPSK off-diagonal at `(2, 0)` is strictly positive.*  Concrete
    `pos`-corollary of `qpsk_exceed_two_zero`. -/
theorem qpsk_exceed_two_zero_pos : 0 < qpsk.exceed 2 0 :=
  lt_of_lt_of_eq zero_lt_one qpsk_exceed_two_zero.symm

/-- *QPSK off-diagonal at `(2, 1)` is strictly positive.*  Concrete
    `pos`-corollary of `qpsk_exceed_two_one`. -/
theorem qpsk_exceed_two_one_pos : 0 < qpsk.exceed 2 1 :=
  lt_of_lt_of_eq zero_lt_one qpsk_exceed_two_one.symm

/-- *QPSK off-diagonal at `(0, 2)` is strictly positive.*  Concrete
    `pos`-corollary of `qpsk_exceed_zero_two`. -/
...
/-- *QPSK off-diagonal at `(1, 0)` is strictly positive.*  Concrete
    `pos`-corollary of `qpsk_exceed_one_zero`. -/
theorem qpsk_exceed_one_zero_pos : 0 < qpsk.exceed 1 0 :=
  lt_of_lt_of_eq zero_lt_one qpsk_exceed_one_zero.symm

/-- *QPSK off-diagonal at `(2, 3)` is strictly positive.*  Concrete
    `pos`-corollary of `qpsk_exceed_two_three`. -/
theorem qpsk_exceed_two_three_pos : 0 < qpsk.exceed 2 3 :=
  lt_of_lt_of_eq zero_lt_one qpsk_exceed_two_three.symm

/-- *QPSK off-diagonal at `(3, 0)` is strictly positive.*  Concrete
    `pos`-corollary of `qpsk_exceed_three_zero`. -/
theorem qpsk_exceed_three_zero_pos : 0 < qpsk.exceed 3 0 :=
  lt_of_lt_of_eq zero_lt_one qpsk_exceed_three_zero.symm

/-- *QPSK exceedance at `(3, 1)` is non-zero.*  `ne_zero`-corollary
    of `qpsk_exceed_three_one`. -/
theorem qpsk_exceed_three_one_ne_zero : qpsk.exceed 3 1 ≠ 0 :=
  fun h => one_ne_zero (qpsk_exceed_three_one.symm.trans h)
...
/-- *QPSK exceedance at `(2, 1)` is non-zero.*  `ne_zero`-corollary
    of `qpsk_exceed_two_one`. -/
theorem qpsk_exceed_two_one_ne_zero : qpsk.exceed 2 1 ≠ 0 :=
  fun h => one_ne_zero (qpsk_exceed_two_one.symm.trans h)

/-- *QPSK exceedance at `(0, 2)` is non-zero.*  `ne_zero`-corollary
    of `qpsk_exceed_zero_two`. -/
...
/-- *QPSK exceedance at `(1, 0)` is non-zero.*  `ne_zero`-corollary
    of `qpsk_exceed_one_zero`. -/
theorem qpsk_exceed_one_zero_ne_zero : qpsk.exceed 1 0 ≠ 0 :=
  fun h => one_ne_zero (qpsk_exceed_one_zero.symm.trans h)


/-- *QPSK exceedance at `(2, 3)` is non-zero.*  `ne_zero`-corollary
    of `qpsk_exceed_two_three`. -/
theorem qpsk_exceed_two_three_ne_zero : qpsk.exceed 2 3 ≠ 0 :=
  fun h => one_ne_zero (qpsk_exceed_two_three.symm.trans h)

/-- *QPSK exceedance at `(3, 0)` is non-zero.*  `ne_zero`-corollary
    of `qpsk_exceed_three_zero`. -/
theorem qpsk_exceed_three_zero_ne_zero : qpsk.exceed 3 0 ≠ 0 :=
  fun h => one_ne_zero (qpsk_exceed_three_zero.symm.trans h)
