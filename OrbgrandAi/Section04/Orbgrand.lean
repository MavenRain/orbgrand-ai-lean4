...
/-- *Bucket non-membership from a witness and strictly smaller index.*
    Specialisation of `landslideBucket_not_of_mem_of_ne` to the
    ordered case `w1 < w2`.  Built by feeding `Nat.ne_of_lt` into the
    `_of_mem_of_ne` variant. -/
theorem landslideBucket_not_of_mem_of_lt
    {n : Nat} (pi : ReliabilityRank n) {w1 w2 : Nat} {e : Fin n -> Bool}
    (h1 : landslideBucket pi w1 e) (h_lt : w1 < w2) :
    ¬ landslideBucket pi w2 e :=
...
/-- *Bucket non-membership at the immediate successor index.*  A
    witness `landslideBucket pi w e` cannot also sit in bucket `w + 1`. -/
theorem landslideBucket_not_of_mem_succ_self
    {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool}
    (h1 : landslideBucket pi w e) :
    ¬ landslideBucket pi (w + 1) e :=
  landslideBucket_not_of_mem_of_lt pi h1 (Nat.lt_succ_self w)

/-- *Bucket non-membership at the immediate predecessor index.*  A
    witness `landslideBucket pi (w + 1) e` cannot also sit in bucket
    `w`.  Mirror of `landslideBucket_not_of_mem_succ_self`. -/
theorem landslideBucket_not_of_mem_pred_self
    {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool}
    (h1 : landslideBucket pi (w + 1) e) :
    ¬ landslideBucket pi w e :=
  landslideBucket_not_of_mem_of_gt pi h1 (Nat.lt_succ_self w)

/-- *Conjunction-form disjointness at the immediate successor index.*
    No pattern `e` can simultaneously witness `landslideBucket pi w e`
    and `landslideBucket pi (w + 1) e`.  Conjunction-form companion to
    `landslideBucket_not_of_mem_succ_self`. -/
theorem landslideBucket_disjoint_succ_self
    {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool} :
...
/-- *Two-step conjunction-form disjointness.*  No pattern `e` can
    simultaneously witness `landslideBucket pi w e` and
    `landslideBucket pi (w + 2) e`.  Conjunction-form companion to
    `landslideBucket_not_of_mem_succ_succ_self`. -/
theorem landslideBucket_disjoint_succ_succ_self
    {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool} :
    ¬ (landslideBucket pi w e /\ landslideBucket pi (w + 2) e) :=
  landslideBucket_disjoint_of_lt pi
    (Nat.lt_add_of_pos_right (Nat.succ_pos 1))

/-- *Two-step conjunction-form disjointness, swapped order.*  No
    pattern `e` can simultaneously witness `landslideBucket pi (w + 2) e`
    and `landslideBucket pi w e`.  Mirror of
    `landslideBucket_disjoint_succ_succ_self`. -/
theorem landslideBucket_disjoint_pred_pred_self
    {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool} :
    ¬ (landslideBucket pi (w + 2) e /\ landslideBucket pi w e) :=
  landslideBucket_disjoint_of_gt pi
    (Nat.lt_add_of_pos_right (Nat.succ_pos 1))

/-- *Bucket non-membership three indices past the witness.*  A
    witness `landslideBucket pi w e` cannot also sit in bucket
    `w + 3`.  Strengthens `landslideBucket_not_of_mem_succ_succ_self`
    by skipping one more successor. -/
theorem landslideBucket_not_of_mem_succ_succ_succ_self
    {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool}
    (h1 : landslideBucket pi w e) :
...
/-- *Bucket non-membership three indices before the witness.*  A
    witness `landslideBucket pi (w + 3) e` cannot also sit in bucket
    `w`.  Mirror of `landslideBucket_not_of_mem_succ_succ_succ_self`. -/
theorem landslideBucket_not_of_mem_pred_pred_pred_self
    {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool}
    (h1 : landslideBucket pi (w + 3) e) :
    ¬ landslideBucket pi w e :=
  landslideBucket_not_of_mem_of_gt pi h1
    (Nat.lt_add_of_pos_right (Nat.succ_pos 2))

/-- *Three-step conjunction-form disjointness.*  No pattern `e` can
    simultaneously witness `landslideBucket pi w e` and
    `landslideBucket pi (w + 3) e`.  Conjunction-form companion to
    `landslideBucket_not_of_mem_succ_succ_succ_self`. -/
theorem landslideBucket_disjoint_succ_succ_succ_self
    {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool} :
...
/-- *Bucket non-membership four indices before the witness.*  A
    witness `landslideBucket pi (w + 4) e` cannot also sit in bucket
    `w`.  Mirror of `landslideBucket_not_of_mem_succ_succ_succ_succ_self`. -/
theorem landslideBucket_not_of_mem_pred_pred_pred_pred_self
    {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool}
...
/-- *Four-step conjunction-form disjointness.*  No pattern `e` can
    simultaneously witness `landslideBucket pi w e` and
    `landslideBucket pi (w + 4) e`.  Conjunction-form companion to
    `landslideBucket_not_of_mem_succ_succ_succ_succ_self`. -/
theorem landslideBucket_disjoint_succ_succ_succ_succ_self
    {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool} :
...
/-- *Five-step conjunction-form disjointness.*  No pattern `e` can
    simultaneously witness `landslideBucket pi w e` and
    `landslideBucket pi (w + 5) e`.  Conjunction-form companion to
    `landslideBucket_not_of_mem_succ_succ_succ_succ_succ_self`. -/
theorem landslideBucket_disjoint_succ_succ_succ_succ_succ_self
    {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool} :
    ¬ (landslideBucket pi w e /\ landslideBucket pi (w + 5) e) :=
  landslideBucket_disjoint_of_lt pi
    (Nat.lt_add_of_pos_right (Nat.succ_pos 4))

/-- *Five-step conjunction-form disjointness, swapped order.*  No
    pattern `e` can simultaneously witness `landslideBucket pi (w + 5) e`
    and `landslideBucket pi w e`.  Mirror of
    `landslideBucket_disjoint_succ_succ_succ_succ_succ_self`. -/
theorem landslideBucket_disjoint_pred_pred_pred_pred_pred_self
    {n : Nat} (pi : ReliabilityRank n) {w : Nat} {e : Fin n -> Bool} :
